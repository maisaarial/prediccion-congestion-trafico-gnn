...
CASOS = ["sensores_tal_cual", "proximidad", "proximidad_comportamiento", "proximidad_sentido_v1", "proximidad_sentido_v2"]
...
def guardar_pred_vs_real(y_pred, y_true, output_path, titulo, max_nodos=5, max_pasos=150):
    """Guarda una gráfica predicción vs real para algunos nodos representativos."""
    ...
    nodos = np.linspace(0, n_nodos - 1, num=min(max_nodos, n_nodos), dtype=int)

    for nodo in nodos:
        plt.figure(figsize=(11, 5))
        plt.plot(range(pasos), y_true[:pasos, nodo], label="Real")
        plt.plot(range(pasos), y_pred[:pasos, nodo], label="Predicción")
...
def guardar_comparaciones_globales(df, output_dir):
    ...
    for modelo in df["modelo"].unique():
        ...
        plt.scatter(tmp["epochs_ejecutadas"], tmp["test_mae"], label=modelo, s=80)
...
def ejecutar_experimento(caso, tipo_adyacencia, nombre_modelo, fraccion_datos, max_epochs, batch_size, patience, hidden_channels, lstm_hidden, data_dir, output_dir):
    print("\n" + "=" * 80)
    print(f"CASO: {caso}")
    ...
    train_data, val_data, test_data, graph_data = cargar_caso(caso, tipo_adyacencia, data_dir)
    ...
    model = construir_modelo(nombre_modelo, graph_data, hidden_channels, lstm_hidden)
    ...
    best_epoch = 0
    epochs_sin_mejora = 0
    history = {"epoch": [], "train_loss": [], "val_loss": [], "val_mae": [], "val_rmse": []}
    inicio = time.time()
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        print(f"Epoch {epoch:03d} | Train loss: {train_loss:.6f} | Val loss: {val_loss:.6f} | Val MAE: {val_mae:.6f} | Val RMSE: {val_rmse:.6f}")
    ...
    test_loss, test_mae, test_rmse, y_pred, y_true = evaluate(model, test_loader, criterion, edge_index, edge_weight, DEVICE)

    exp_name = f"{caso}_{tipo_adyacencia}_{nombre_modelo}_datos_{int(fraccion_datos * 100)}_epochs_{max_epochs}"
    output_dir = Path(output_dir)
    ...
    pd.DataFrame(history).to_csv(historiales_dir / f"{exp_name}.csv", index=False)
    guardar_curva_entrenamiento(history, curvas_dir / f"{exp_name}.png", exp_name)
    guardar_pred_vs_real(y_pred, y_true, pred_dir / f"{exp_name}.png", exp_name)
...
def main():
    parser = argparse.ArgumentParser(description="Entrena y compara GCN_LSTM, GCN_GRU y GAT_LSTM por caso y adyacencia.")
            ...
            for modelo in args.modelos:
                for fraccion in args.fracciones:
                    for max_epochs in args.epochs:
                        try:
                            res = ejecutar_experimento(caso, tipo_ady, modelo, fraccion, max_epochs, args.batch_size, args.patience, args.hidden_channels, args.lstm_hidden, args.data_dir, args.output_dir)
                            resultados.append(res)
                            pd.DataFrame(resultados).to_csv(Path(args.output_dir) / "resultados_completos.csv", index=False)
                        except FileNotFoundError as e:
                            print(f"\nArchivo no encontrado. Se omite: caso={caso}, adyacencia={tipo_ady}, modelo={modelo}. {e}")
                        except RuntimeError as e:
                            print(f"\nERROR EN EXPERIMENTO: caso={caso}, adyacencia={tipo_ady}, modelo={modelo}. {e}")
                            if "out of memory" in str(e).lower() and torch.cuda.is_available():
                                torch.cuda.empty_cache()
                        except Exception as e:
                            print(f"\nERROR no controlado: caso={caso}, adyacencia={tipo_ady}, modelo={modelo}. {e}")

    df = pd.DataFrame(resultados)
    if not df.empty:
        df_ordenado = df.sort_values(by=["test_mae", "test_rmse"], ascending=True)
        df_ordenado.to_csv(Path(args.output_dir) / "ranking_modelos.csv", index=False)
        ...
        print("No se generaron resultados. Revisa data/processed o ejecuta scripts/generar_datasets.py primero.")
